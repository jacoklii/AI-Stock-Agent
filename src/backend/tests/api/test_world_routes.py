...
The contract: ``GET /world`` reuses the latest digest as the overview, groups stored (scraper-swept)
events into the four ordered domains via the keyword classifier, and splits the ranked signals into
Now / Building by horizon. The session is faked: the first ``execute`` is the digest scan, the
second is the news-event scan.
"""
...
def test_world_groups_events_into_four_ordered_domains(client) -> None:
    ...
    use_session(FakeSession([FakeResult(scalars=[]), FakeResult(scalars=events)]))
    ...
    # Stored events are scraper-swept; significance is never echoed as a number.
